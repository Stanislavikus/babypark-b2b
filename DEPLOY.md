...
GAP-026B workspace RBAC authority cutover.
...
3. deploy exact approved 026B code while traffic is blocked;
4. run pending migrations;
5. seed canonical `workspace_permissions` catalogue (`WorkspaceRbacPermissionSeeder`);
6. run guarded RBAC cutover **CHECK-ONLY**;
7. if safe: **EXECUTE** deterministic legacy backfill;
...
authority fallback; no role-based Connector/Tax/Mapping fallback; reconcile before
traffic resumes.
...
- **CHECK-ONLY** — diagnostics only; no RBAC assignments; may run before maintenance;
- **EXECUTE** — requires maintenance/quiesced merchant writes; re-runs preflight;
