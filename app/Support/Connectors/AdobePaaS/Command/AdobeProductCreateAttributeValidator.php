...
final class AdobeProductCreateAttributeValidator
{
    /** @var list<string> */
    ...
    public function validate(
        ...
        AdobeProductDesiredState|AdobeProductParentDesiredState $desiredState,
        string $productType,
        ...
        if ($membershipCount !== count($metadata->attributes)
            && $this->hasPotentialRequiredMetadataGap(
                $workspaceId,
                $connectorAccountId,
                $set->id,
        ...
        foreach ($metadata->attributes as $code => $attribute) {
            if (
                ...
                || ! $this->appliesTo($attribute, $productType)
            ...
            if ($productType === 'configurable' && $code === 'price') {
                continue;
            }
     ...
     */
    private function hasPotentialRequiredMetadataGap(
        string $workspaceId,
        string $connectorAccountId,
        ...
        foreach ($memberships as $membership) {
            ...
            if ($applyTo !== [] && ! in_array($productType, $applyTo, true)) {
                continue;
            }
            ...
            // Required=true or unknown on an applicable membership must fail closed.
            return true;
        }
    ...
    private function appliesTo(AdobeAttributeMetadata $attribute, string $productType): bool
    {
        return $attribute->applyTo === [] || in_array($productType, $attribute->applyTo, true);
    }

    private function valueFor(string $code, AdobeProductDesiredState|AdobeProductParentDesiredState $desiredState): mixed
    {
        if (in_array($code, self::CORE_FIELDS, true)) {
            return match ($code) {
                ...
                'price' => $desiredState instanceof AdobeProductDesiredState ? $desiredState->price : null,
            };
        }
