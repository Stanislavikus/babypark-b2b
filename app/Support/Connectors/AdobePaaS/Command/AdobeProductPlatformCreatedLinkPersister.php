...
final class AdobeProductPlatformCreatedLinkPersister
{
    ...
    public function persistProductLink(
        string $workspaceId,
        string $connectorAccountId,
        string $productId,
        string $sku,
        ...
        if (! ctype_digit($productId) || (int) $productId <= 0) {
            throw new AdobeProductPlatformCreatedLinkPersistenceException('platform_created_product_id_invalid');
        }
        ...
        return DB::transaction(function () use ($workspaceId, $connectorAccountId, $localProductId, $sku, $logicalEntityId): ExternalRecordLink {
            ConnectorAccount::withoutWorkspaceScope()
                ->where('workspace_id', $workspaceId)
                ->whereKey($connectorAccountId)
                ->lockForUpdate()
            ...
            if ($existing instanceof ExternalRecordLink) {
                if ($existing->hasTrustedIdentity()
                    && (string) $existing->external_identifier === $sku
                    && (string) $existing->external_record_discriminator === (string) $logicalEntityId
                ) {
                    return $existing;
            ...
            if ($this->linkGuard->hasParentDiscriminatorCrossSubjectCollision(
                $workspaceId,
                $connectorAccountId,
                (string) $logicalEntityId,
                (string) $localProductId,
