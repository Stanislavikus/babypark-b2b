...
final class FieldMappingReadModelProjector
{
    ...
    private function compareInternalRows(
        ...
        $leftBinding = $this->findBindingById($bindings, $left->fieldBindingId);
        $rightBinding = $this->findBindingById($bindings, $right->fieldBindingId);

        $leftSortOrder = $leftBinding?->sort_order ?? 0;
        $rightSortOrder = $rightBinding?->sort_order ?? 0;
     ...
     * @param  list<FieldBinding>  $bindings
     */
    private function findBindingById(array $bindings, string $bindingId): ?FieldBinding
    {
        foreach ($bindings as $binding) {
