...
final class MagentoV1ModulelessArchitectureInvariantTest extends TestCase
{
    ...
    #[Test]
    public function configurable_family_lock_ends_before_media_and_category_and_runtime_has_no_rollback_delete(): void
    {
        $live = File::get(base_path('app/Support/Connectors/AdobePaaS/AdobeProductExportLiveCapability.php'));
        $coordinator = File::get(base_path('app/Support/Connectors/AdobePaaS/Command/AdobeConfigurableProductCommandCoordinator.php'));
        $parent = File::get(base_path('app/Support/Connectors/AdobePaaS/Command/AdobeConfigurableParentCommandExecutor.php'));
        $option = File::get(base_path('app/Support/Connectors/AdobePaaS/Command/AdobeConfigurableOptionCommandExecutor.php'));
        $child = File::get(base_path('app/Support/Connectors/AdobePaaS/Command/AdobeConfigurableChildLinkCommandExecutor.php'));

        $this->assertLessThan(strpos($live, 'mediaLiveExecutor->executeAfterCoreProduct'), strpos($live, 'configurableCoordinator->execute'));
        $this->assertLessThan(strpos($live, 'categoryRelationExecutor->executeAfterProduct'), strpos($live, 'mediaLiveExecutor->executeAfterCoreProduct'));
        $this->assertStringContainsString('ConnectorAccountOperationLock::cacheKey', $coordinator);

        foreach ([$coordinator, $parent, $option, $child] as $runtime) {
            $this->assertStringNotContainsString('deleteProduct', $runtime);
            $this->assertStringNotContainsString('deleteConfigurable', $runtime);
        }
    }
}
