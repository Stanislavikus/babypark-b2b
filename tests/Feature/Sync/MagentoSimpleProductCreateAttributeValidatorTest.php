...
use App\Support\Connectors\AdobePaaS\Command\AdobeProductCreateAttributeValidator;
...
final class MagentoSimpleProductCreateAttributeValidatorTest extends TestCase
{
    ...
    #[Test]
    public function configurable_parent_does_not_require_price_create_input(): void
    {
        [$account, $source, $snapshot, $set] = $this->structure();
        $this->attribute(
            $account->workspace_id,
        ...
        $result = app(AdobeProductCreateAttributeValidator::class)->validate(
            $account->workspace_id,
            $account->id,
            new AdobeProductParentDesiredState(1, 'CFG-PRICELESS', 'Configurable', 9, 'configurable', 2, 4, []),
            'configurable',
        );
