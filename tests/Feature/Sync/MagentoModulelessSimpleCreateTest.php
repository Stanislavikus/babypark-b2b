...
use App\Support\Connectors\AdobePaaS\Command\AdobeProductPlatformCreatedLinkPersistenceException;
use App\Support\Connectors\AdobePaaS\Command\AdobeProductPlatformCreatedLinkPersister;
...
final class MagentoModulelessSimpleCreateTest extends TestCase
{
    ...
    #[Test]
    public function configurable_parent_platform_created_link_is_persisted_idempotently(): void
    {
        $workspace = $this->defaultWorkspace();
        $account = $this->createConnectorAccount($workspace);
        [$product] = $this->createProductVariant($workspace, 'PARENT-LINK-CHILD');
        $persister = app(AdobeProductPlatformCreatedLinkPersister::class);

        $first = $persister->persistProductLink($workspace->id, $account->id, (string) $product->id, 'CFG-PARENT', 901);
        $second = $persister->persistProductLink($workspace->id, $account->id, (string) $product->id, 'CFG-PARENT', 901);

        $this->assertTrue($first->is($second));
        $this->assertSame(ExternalRecordLinkTrustOrigin::PlatformCreated->value, $first->trust_origin);
        $this->assertSame('901', $first->external_record_discriminator);
        $this->assertNull($first->established_by_workspace_user_id);
    }

    #[Test]
    public function configurable_parent_platform_created_link_fails_closed_on_sku_collision(): void
    {
        $workspace = $this->defaultWorkspace();
        $account = $this->createConnectorAccount($workspace);
        [$product, $variant] = $this->createProductVariant($workspace, 'COLLIDING-SKU');
        ExternalRecordLink::withoutWorkspaceScope()->create([
            'workspace_id' => $workspace->id,
            'connector_account_id' => $account->id,
            'product_variant_id' => $variant->id,
            'external_identifier' => 'CFG-COLLISION',
            'trust_origin' => ExternalRecordLinkTrustOrigin::PlatformCreated->value,
            'external_record_discriminator' => '902',
            'established_at' => now(),
        ]);

        $this->expectException(AdobeProductPlatformCreatedLinkPersistenceException::class);
        app(AdobeProductPlatformCreatedLinkPersister::class)->persistProductLink(
            $workspace->id,
            $account->id,
            (string) $product->id,
            'CFG-COLLISION',
    ...
    #[Test]
    public function configurable_parent_platform_created_link_fails_closed_on_discriminator_collision(): void
    {
        $workspace = $this->defaultWorkspace();
        $account = $this->createConnectorAccount($workspace);
        [$existingProduct] = $this->createProductVariant($workspace, 'EXISTING-PARENT-CHILD');
        [$targetProduct] = $this->createProductVariant($workspace, 'TARGET-PARENT-CHILD');
        ExternalRecordLink::withoutWorkspaceScope()->create([
            'workspace_id' => $workspace->id,
            'connector_account_id' => $account->id,
            'product_id' => $existingProduct->id,
            'external_identifier' => 'CFG-EXISTING',
            'trust_origin' => ExternalRecordLinkTrustOrigin::PlatformCreated->value,
            'external_record_discriminator' => '904',
            'established_at' => now(),
        ]);
    ...
    #[Test]
    public function standalone_simple_create_requires_the_account_operation_lock(): void
    {
        $workspace = $this->defaultWorkspace();
        $account = $this->createConnectorAccount($workspace);
        [, $variant] = $this->createProductVariant($workspace, 'LOCKED-CREATE');
        $this->createAttributeSet($account->workspace_id, $account->id, $account->connector_definition_id, 4);
        $lock = Cache::lock(ConnectorAccountOperationLock::cacheKey($account->id), 30);
        ...
        $this->assertSame('product_create_account_lock_timeout', $result->evidence->reasonCode);
    }
