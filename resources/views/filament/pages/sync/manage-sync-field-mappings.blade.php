                  ...
                  <div class="flex flex-wrap justify-end gap-2">
                    @if ($row['semantic_state'] === 'suggested')
                      <x-filament::button
                        size="xs"
                        color="primary"
                        wire:click="confirmMapping('{{ $row['field_binding_id'] }}', '{{ $row['suggested_external_field_key'] }}')"
                    ...
                    @if ($row['semantic_state'] === 'mapped' || $row['semantic_state'] === 'needs_attention')
                      <x-filament::button
                        size="xs"
                        color="gray"
                        wire:click="mountAction('changeMapping', { fieldBindingId: '{{ $row['field_binding_id'] }}', externalFieldKey: '{{ $row['existing_external_field_key'] }}' })"
                      ...
                      <x-filament::button
                        size="xs"
                        color="danger"
                        wire:click="removeMapping('{{ $row['field_binding_id'] }}', '{{ $row['existing_external_field_key'] }}')"
                    ...
                    @if ($row['semantic_state'] === 'unmapped')
                      <x-filament::button
                        size="xs"
                        color="gray"
                        wire:click="mountAction('changeMapping', { fieldBindingId: '{{ $row['field_binding_id'] }}', externalFieldKey: '' })"
