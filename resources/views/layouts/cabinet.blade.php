    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="flex h-16 items-center justify-between">
            <div class="flex items-center gap-6">
                <a href="{{ route('cabinet.catalog') }}" class="text-lg font-bold text-primary-600">
                    BabyPark B2B
                </a>
                <a href="{{ route('cabinet.dashboard') }}"
                   class="text-sm font-medium transition-colors hover:text-primary-600 {{ request()->routeIs('cabinet.dashboard') ? 'text-primary-600' : 'text-gray-700' }}">
                    Кабінет
                </a>
                <a href="{{ route('cabinet.catalog') }}"
                   class="text-sm font-medium transition-colors hover:text-primary-600 {{ request()->routeIs('cabinet.catalog*') ? 'text-primary-600' : 'text-gray-700' }}">
                    Каталог
                </a>
            </div>
...
<div id="bp-photo-lb"
    ...
    <div style="position:relative;max-width:min(800px,90vw);background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 25px 60px rgba(0,0,0,.4);">
        ...
        <div style="padding:16px;display:flex;justify-content:center;">
            <img id="bp-photo-lb-img" src="" alt=""
