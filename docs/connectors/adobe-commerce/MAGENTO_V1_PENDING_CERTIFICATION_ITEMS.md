...
- Fail-closed/unsupported boundaries: missing option CREATE remains `configurable_option_create_not_certified`; destructive option-value removal remains `configurable_option_value_removal_requires_adobe_validation`; remote child unlink/removal is not exposed as a production capability; Product CREATE and blind consequential retry remain forbidden.
