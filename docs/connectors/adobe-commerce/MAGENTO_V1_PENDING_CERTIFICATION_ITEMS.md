...
- Current truth: independent review surfaced Magento issue class #8897/#26484: saving through `ProductRepository` in a store-view context has historically copied untouched inherited EAV values into store-specific rows, disabling `Use Default Value`. Upstream #26484 is now Closed and labeled `Fixed in 2.4.x`, but that is not sufficient closure evidence for the distinct #8897 behavior: #8897 is Closed/Done, still labeled confirmed/reproduced on 2.4.x/latest 2.4-develop, and exposes no linked fix PR. Our stock REST path ultimately saves through ProductRepository, so this cannot be dismissed from code inspection alone.
- 2026-09-12 target probe: the configured `default` context is real store view id `1`; the installation exposes eight active storefront views plus Admin id `0`. Product GET succeeds through `default`, `all`, and every active storefront view, while `/rest/admin/V1/products/{sku}` returns HTTP 400. `name`, `meta_title`, and `meta_description` are confirmed `scope=store` by live attribute metadata. The optional Safe Sync handshake is HTTP 404 on this target, so it cannot supply a raw-EAV diagnostic. Adobe documents `all` writes as updating all store scopes, therefore `all` cannot be repurposed as a global-only inheritance control. Evidence: `docs/connectors/adobe-commerce/magento_v1_store_scope_inheritance_probe_2026_09_12.json`.
- Why still pending: ordinary Product GET returns effective values and therefore cannot prove whether equal-valued store-specific override rows were newly materialized. Direct Magento DB/Admin `Use Default Value` evidence is not available from the current certification control plane, and no installed target-side raw-scope diagnostic is available. A blind Product PUT was deliberately **not** executed because, without an authoritative before/after seam, it would be non-probative and could itself create the hidden rows under test. Previous write/read/restore certification proves controlled values and no observable payload drift, not absence of hidden scope-row side effects.
- Needed proof before public Live support: obtain direct DB/Admin `Use Default Value` evidence or a temporary validation-only target-side raw-EAV diagnostic; on the exact supported Magento version establish a baseline for one or more inherited store/website-scoped attributes, perform exactly one ordinary stock Product PUT through configured store code `default`, and prove untouched attributes did not gain overrides. If stock REST cannot preserve inheritance reliably, standard V1 must fail closed for affected store-view writes or narrow support to a scope/path with proven safety.
