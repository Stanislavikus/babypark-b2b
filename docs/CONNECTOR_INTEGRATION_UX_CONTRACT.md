...
**Non-goal:** This contract does not itself authorize arbitrary backend work and is not the workspace-permission implementation specification beyond the approved domain authorization contract in `docs/03-DOMAIN_MODEL.md` → **Workspace access model and authorization (Resolved — Task 4C-1c-2a)**. Some underlying mechanisms are already shipped (for example `ConnectorCapability`, Discovery runtime, snapshot persistence, workspace isolation guards). Missing backend/runtime/security prerequisites require their own scoped tasks. Specifically, mutable Layer-B mapping UI may ship only after verified **GAP-026B-2** repository merge and successful production maintenance-window **EXECUTE** — repository work may proceed after B-2 merge; merchant traffic under new authority remains blocked until environment activation. Do **not** claim that historical pre-B-2 fixed `User.role` authorization satisfies this UX contract — that transitional behavior is historical evidence only under **GAP-026** / PR #102.
