...
- `docs/connectors/adobe-commerce/magento_v1_receive_r4_dynamic_select_evidence_2026_09_13.json` — R4 code/test closure evidence: exact option reverse resolution, Dynamic CAS, stale-state guards, RemoteAbsent≠Clear, broad Connector/Sync gates; supplemented by the 2026-09-19 real-target certification below.
- `docs/connectors/adobe-commerce/magento_v1_receive_r4_real_target_certification_2026_09_19.json` — P-07 closure evidence for real Magento Dynamic single-select Receive on a Variant-subject Simple target: exact option-ID reverse mapping, real stale-local fail-closed proof, successful Apply, remote/local restore, and fixture cleanup; public Products/Import/Live remains false.
...
- `docs/reviews/PRODUCT_WORKBENCH_SONNET_ARBITRATION_2026_09_21.md` — Lead arbitration of the independent Sonnet 5 High challenge; accepts/rejects findings against frozen repo truth, records the Magento-default-row-universe Stop-and-Amend need, and keeps richer Product lifecycle semantics open.
- `docs/reviews/PRODUCT_WORKBENCH_GPT54_TARGETED_RESEARCH_TASK_2026_09_21.md` — targeted GPT-5.4 repo/architecture research task for the remaining structural questions only: Magento workspace row universe, Product draft/lifecycle semantics, scalable Remote Catalogue Index V2, category/Attribute Set preparation, readiness, saved Views, settings inheritance, and automated price/stock path.
...
- `docs/reviews/PRODUCT_WORKBENCH_AB_SONNET_FINAL_ARBITRATION_2026_09_21.md` — final Lead arbitration of Sonnet confirmation; records no blockers, accepts two factual corrections (Remote Catalogue projection limits and planner rewiring scope), and declares A+B ready for product-owner approval.
- `docs/reviews/PRODUCT_WORKBENCH_STRUCTURAL_CONTRACT_2026_09_21.md` — **[Resolved — 2026-09-21; A3 amended 2026-09-22]** Product-owner-approved A+B structural contract. Freezes remote `Огляд` vs local `Публікація` row universes, keeps correspondence governed by ExternalRecordLink inside Overview for first UX, and freezes default+sparse Category/Attribute-Set classification, Projection V2 boundary, planner migration boundary, and Preview/Live determinism rules.
...
- `docs/reviews/PRODUCT_WORKBENCH_VISUAL_UX_FINAL_ARBITRATION_2026_09_21.md` — **historical visual arbitration** of corrected Hybrid commit `7df5fce…`; verifies V-01..V-08 were closed, but its three-tab navigation is superseded by the [Resolved — 2026-09-22] visual contract.
- `docs/reviews/PRODUCT_WORKBENCH_VISUAL_UX_CONTRACT_2026_09_22.md` — **[Resolved — 2026-09-22; visual Stop-and-Amend 2026-09-23]** merchant-facing Workbench visual/interaction contract. Supersedes the three-tab prototype target: top-level `Огляд | Публікація`, correspondence inside Overview, native compact SaaS Focus Mode, standard table-filter surface (no permanent second filter rail in first scope), compact one-line header/tab strip, `Стан даних`/readiness/problems separation, AI-ready field foundation, and Master-Product-only creation direction.
- `docs/reviews/PRODUCT_WORKBENCH_REMOTE_CATALOGUE_PROJECTION_V2_REAL_MAGENTO_STUDY_2026_09_22.md` — Lead READ-only real-Magento study proving Product list payload already carries Attribute Set, category links, media/thumbnail evidence and provider attributes without per-Product GET N+1; records observed response-size evidence and recommends a richer bounded Projection V2 before building the target Overview.
- `docs/reviews/PRODUCT_WORKBENCH_CLASSIFICATION_CATALOGUE_REAL_MAGENTO_EVIDENCE_2026_09_24.md` — READ-only real-target evidence for the resolved complete classification-catalogue addendum: 28 Magento Category rows vs only 2 category IDs currently used by 19 Products, 22 active selectable empty Categories, 4 current Attribute Sets with 2 unused, root `is_active` omission, and confirmed standard-REST Attribute→Group placement limitation.
- `docs/reviews/PRODUCT_WORKBENCH_PROJECTION_V2_IMPLEMENTATION_TASK_DRAFT_2026_09_22.md` — draft YELLOW executor task for the next technical dependency: enrich the bounded immutable Remote Catalogue projection with useful Overview fields, preserve response-size safety and no-N+1 behavior, then prove it on the real Magento target.
- `docs/reviews/PRODUCT_WORKBENCH_UX1_IMPLEMENTATION_TASK_DRAFT_2026_09_21.md` — updated draft YELLOW executor task for the post-Projection-V2 Workbench shell: Focus Mode + `Огляд | Публікація`, correspondence inside Overview, Publication on current selection/Preview runtime, with no new persistence or unsupported capabilities.
