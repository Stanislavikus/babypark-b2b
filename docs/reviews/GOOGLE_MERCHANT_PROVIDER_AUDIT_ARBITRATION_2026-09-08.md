...
## Correction pass implementation status

Implemented on branch `fix/google-provider-semantic-corrections` from authoritative `develop @ 47906e741afa0e19ca3215869b14698632fa7cb2`.

Applied correction set:
- current Products v1 SKU mapping now uses literal external key `offerId`; ProductInput surface remains in applicability/evidence;
- both legacy `identifier_exists` and current `identifierExists` use the correct false-on-genuine-absence polarity;
- Google `productWeight` / `shippingWeight` mappings are downgraded to `partially_verified` pending DEC-009 packaging-level proof;
- `structuredTitle` / `structuredDescription` are deferred structured-content + digital-source-provenance concepts with a dedicated disagreement;
- `googleProductCategory` and `productTypes` retain separate Connector-owned concepts with distinct taxonomy/category-path representations;
- `numberOfUnits` keeps PropertyVertical ownership but is marked as vertical availability quantity;
- `google_landing_url` now covers only unresolved `canonicalLink` / `mobileLink`; primary `url -> link` remains verified transformed mapping;
- regression coverage proves no Google mapping for canonical `gender=other` and fail-closed legacy/current exact-key behavior for `id` vs `offerId`.

Local validation after correction:
- Google generate + validate: 156/156 physical rows, 156 concepts, zero silent drops;
- Canonical Registry validator: 0 errors;
- Google targeted unit corrections: 14 tests / 2,548 assertions PASS;
- FieldMappingSuggestionReadModel full file: 28 tests / 87 assertions PASS;
- full CanonicalCoverage suite: 87 tests / 31,417 assertions PASS.

The earlier two CanonicalCoverage integrity failures were reproduced as environment-only: `/tmp` tmpfs was 100% full, causing truncated temporary corpora. After deleting only test-generated `/tmp/canonical-coverage-*` and `/tmp/google-coverage-*` directories, the unchanged integrity tests pass. No production or test-contract workaround was added.
