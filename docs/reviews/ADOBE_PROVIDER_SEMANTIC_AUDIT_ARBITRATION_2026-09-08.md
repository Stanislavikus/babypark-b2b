...
- Deeper same-pattern verification found that `additional_images`, `additional_image_labels`, and `hide_from_product_page` are also Import representations rather than literal REST product fields. They map structurally to media-gallery entry `file`, `label`, and `disabled` members through list/member translation.
- `swatch_image` and `media_video` were deliberately not generalized from this pattern: current evidence does not justify treating `swatch_image` as one of the documented `types` role tokens, and video has a separate `video_content` structure.
...
1. **Media representation:** split/qualify CSV image-role keys vs REST media-role tokens and explicitly relate Import additional-image/list/visibility fields to REST gallery `file`/`label`/`disabled` members; preserve Media ownership and current correct runtime behavior.
