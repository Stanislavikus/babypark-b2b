...
2026-09-23 product-owner clarification: `Фільтри` and `Стовпці` use the same right-side slide-over interaction; count badges use the project-standard native Filament corner placement on the trigger, and the search field aligns to the same left grid inset as the table content. The Workbench connector identity uses the official Magento mark rather than a fabricated letter tile. The mark is constrained to the outer height of the compact utility button (`Оновити`) so connector branding never dominates the Workbench header.
