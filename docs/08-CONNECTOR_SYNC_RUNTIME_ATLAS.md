...
| `SyncLiveAdmissionService` | IMPLEMENTED (Stage 3A shell) | Connector | `app/Services/Sync/SyncLiveAdmissionService.php` | `sync_runs` | `tests/Feature/Sync/Stage3ALiveSafetyFoundationTest.php` | `docs/03-DOMAIN_MODEL.md` → Stage 3-0 Preview→Live trust | platform-reusable |
| Live execution job shell (`tries = 1`) | IMPLEMENTED (Stage 3A shell — fail-closed, no Adobe write) | Connector | `app/Jobs/Connectors/SyncLiveRunJob.php` | `sync_runs` | `tests/Feature/Sync/Stage3ALiveSafetyFoundationTest.php` | `docs/03-DOMAIN_MODEL.md` → Stage 3-0 stale active-run safety | platform-reusable |
...
| Adobe Product Export Live capability (Stage 3B simple) | IMPLEMENTED (internal; support false) | Connector | `app/Support/Connectors/AdobePaaS/AdobeProductExportLiveCapability.php`; `AdobeProductExportRunMetadataPreparer.php`; `AdobeStoreConfigReader.php` — current metadata + proven `base_currency_code` via `GET /V1/store/storeConfigs?storeCodes[]=…`; shared semantic planner; simple command executor; Live outcome mapping | `sync_run_items` | `tests/Feature/Sync/Stage3B3AdobeSimpleLiveIntegrationTest.php`; `tests/Unit/Connectors/AdobePaaS/AdobeStoreConfigReaderTest.php` | `docs/03-DOMAIN_MODEL.md` → **3B — Adobe Simple Live** | connector-owned |
...
| Live execution | IMPLEMENTED (Stage 3B generic orchestration — Adobe simple binding internal; no merchant Live UI; support false) | Connector | `SyncLiveAdmissionService`; `SyncLiveRunJob`; `SyncLiveConnectorCapabilityResolver`; Adobe `live_capability` | `sync_runs`; `sync_run_items` | `tests/Feature/Sync/Stage3ALiveSafetyFoundationTest.php`; `tests/Feature/Sync/Stage3B3AdobeSimpleLiveIntegrationTest.php` | `docs/03-DOMAIN_MODEL.md` → Stage 3-0 / Stage 3A–3E; merchant consequential exposure gated on `ConnectorSyncOperationSupport(Products, Export, Live) === true` until Stage 3E flip | connector-owned transport; platform-reusable outcomes |
