...
| Adobe Product desired-state compilation (Stage 3B-2) | IMPLEMENTED (unwired) | Connector | `app/Support/Connectors/AdobePaaS/Command/AdobeProductDesiredStateCompiler.php`; `AdobeProductDesiredState`; `AdobeProductCommandCompilationException` — compiles `AdobeProductExportSemanticResult` only; blocks on semantic findings; uses `resolved_price.effective_net_price` + currency; protects connector-owned top-level fields | none | `tests/Unit/Connectors/AdobePaaS/Command/AdobeProductCommandFoundationUnitTest.php`; `tests/Feature/Sync/Stage3B2AdobeSimpleCommandSafetyTest.php` | `docs/03-DOMAIN_MODEL.md` → Live Safety, Identity & First-Live Contract (Stage 3B-2) | connector-owned |
