...
|---|---|---|---|---|---|---|---|
| Workspace RBAC catalogue (7 atomic permissions) | IMPLEMENTED | Core | `app/Support/Workspace/WorkspacePermissions.php`; `app/Services/Workspace/WorkspaceAuthorization.php`; `database/seeders/WorkspaceRbacPermissionSeeder.php` | `workspace_permissions` | `tests/Feature/WorkspaceRbacCatalogueSeederTest.php`; `tests/Feature/WorkspaceAuthorizationTest.php` | `docs/03-DOMAIN_MODEL.md` → Workspace RBAC | platform-reusable |
| `run_sync_preview` permission | RESOLVED — NOT IMPLEMENTED | Core | *(not in `WorkspacePermissions::catalogue()`)* | none | `tests/Feature/Sync/PreviewSyncExecutionFoundationDocumentationContractTest.php`; `tests/Feature/Sync/SyncRunPersistenceFoundationTest.php` | `docs/03-DOMAIN_MODEL.md` → Preview execution permission | platform-reusable |
...
| SyncConfiguration | IMPLEMENTED | Connector | `app/Models/SyncConfiguration.php`; `app/Services/Sync/SyncConfigurationService.php` | `sync_configurations` | `tests/Feature/Sync/SyncConfigurationFoundationTest.php` | `docs/03-DOMAIN_MODEL.md` → SyncConfiguration | platform-reusable |
| SyncConfiguration merchant reachability | RESOLVED — NOT IMPLEMENTED | Connector | mapping URL requires `{account}/{configuration}` UUIDs: `app/Filament/Pages/Sync/ManageSyncFieldMappings.php` | `sync_configurations` | `tests/Feature/Sync/ManageSyncFieldMappingsPageTest.php` | `docs/03-DOMAIN_MODEL.md` → Magento V1 / SyncConfiguration merchant reachability | platform-reusable |
| Configuration revision v3 | IMPLEMENTED | Connector | `app/Support/Sync/SyncConfigurationRevisionHasher.php`; `database/migrations/2026_08_16_100000_sync_configuration_revision_v3.php` | `sync_configurations.configuration_revision` | `tests/Feature/Sync/SyncRunPersistenceFoundationTest.php` | `docs/03-DOMAIN_MODEL.md` → Revision v3 | platform-reusable |
...
| Execution support (binary pair) | IMPLEMENTED | Connector | `app/Support/Connectors/ConnectorSyncOperationSupport.php`; `app/Support/Connectors/ConnectorSyncSupportResolver.php` | none | `tests/Feature/Sync/SyncConfigurationFoundationTest.php` | `docs/03-DOMAIN_MODEL.md` → Adobe operation-support truth | platform-reusable / family-hypothesis |
| Mode-aware execution support (Preview vs Live) | RESOLVED — NOT IMPLEMENTED | Connector | current interface has no `execution_mode`; Adobe adapter does not implement support | none | `tests/Feature/Sync/PreviewSyncExecutionFoundationDocumentationContractTest.php` | `docs/03-DOMAIN_MODEL.md` → Magento V1 / Execution support truth | platform-reusable |
...
|---|---|---|---|---|---|---|---|
| Product + 0..N ProductVariant aggregate | RESOLVED — NOT IMPLEMENTED | required | required (simple + configurable) | required (Product/ProductVariant) | offer/item projection | row may flatten; core stays Product | platform-reusable / family-hypothesis |
...
| Run admission / one active run | RESOLVED — NOT IMPLEMENTED | required | required | required | required | required | platform-reusable / family-hypothesis |
| Normalized Preview/Live outcomes | RESOLVED — NOT IMPLEMENTED | required | required | required | required | required | platform-reusable / family-hypothesis |
| Mode-aware execution support | RESOLVED — NOT IMPLEMENTED | required | required | required | required | required | platform-reusable / family-hypothesis |
