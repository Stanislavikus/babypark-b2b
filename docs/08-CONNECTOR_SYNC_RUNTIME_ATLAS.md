...
| `manage_sync_configurations` permission | RESOLVED — NOT IMPLEMENTED | Core | *(absent — normative ninth permission frozen Stage 2-0; runtime pending Stage 2A)* | none | `tests/Feature/Sync/MerchantPreviewAuthorizationRemediationDocumentationContractTest.php` | `docs/03-DOMAIN_MODEL.md` → Merchant Preview Authorization & Remediation Contract | platform-reusable |
...
| SyncConfiguration non-mutating existence lookup (Preview-only actors) | RESOLVED — NOT IMPLEMENTED | Connector | *(absent — Stage 2A required; must not call `ensure*()` helpers)* | none | `tests/Feature/Sync/MerchantPreviewAuthorizationRemediationDocumentationContractTest.php` | `docs/03-DOMAIN_MODEL.md` → Merchant Preview Authorization & Remediation Contract | platform-reusable |
...
| Adobe Product Export execution configuration | IMPLEMENTED | Connector | `app/Support/Connectors/AdobePaaS/AdobeProductExportExecutionConfiguration.php`; `app/Services/Sync/AdobeProductExportSetupService.php` (`configureAttributeSet`) | `sync_configurations.connector_execution_configuration` | `tests/Unit/Connectors/AdobePaaS/AdobeProductExportMetadataReaderTest.php`; `tests/Unit/Connectors/AdobePaaS/AdobeProductExportSetupServiceTest.php`; `tests/Feature/Sync/SyncPreviewAdmissionTest.php` | `docs/03-DOMAIN_MODEL.md` → E5 Adobe attribute-set ownership | connector-owned |
