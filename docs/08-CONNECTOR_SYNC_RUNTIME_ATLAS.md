...
| Merchant Preview work surface (Stage 2A-2) | IMPLEMENTED | Connector UX | `app/Filament/Pages/Sync/ManageAdobeProductsExportPreview.php`; `app/Filament/Pages/Sync/ListSyncDataSetup.php`; `app/Services/Sync/SyncPreviewMerchantReadService.php`; `app/Support/Sync/Preview/Presentation/*` | reads `sync_runs`; `sync_run_items` | `tests/Feature/Sync/Stage2A2MerchantPreviewWorkSurfaceTest.php` | `docs/03-DOMAIN_MODEL.md` → Merchant Preview Authorization & Remediation Contract | Adobe Products Export first slice |
