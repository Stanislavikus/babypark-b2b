...
| `run_sync_live` permission | IMPLEMENTED (Stage 3A) | Core | `app/Support/Workspace/WorkspacePermissions.php`; `database/seeders/WorkspaceRbacPermissionSeeder.php` | `workspace_permissions` | `tests/Feature/Sync/RunSyncLivePermissionTest.php` | `docs/03-DOMAIN_MODEL.md` → Live Safety, Identity & First-Live Contract (Stage 3-0) | platform-reusable |
| Stale active-run recovery | IMPLEMENTED (Stage 3A) | Connector | `app/Services/Sync/SyncRunActiveRecoveryService.php`; `config/sync_runtime.php` | `sync_runs` lease columns | `tests/Feature/Sync/Stage3ALiveSafetyFoundationTest.php` | `docs/03-DOMAIN_MODEL.md` → Stage 3-0 stale active-run safety | platform-reusable |
...
| ExternalRecordLink | IMPLEMENTED (Stage 3A foundation — persistence only) | Connector | `app/Models/ExternalRecordLink.php`; `database/migrations/2026_08_19_110000_external_record_links.php` | `external_record_links` | `tests/Feature/Sync/ExternalRecordLinkFoundationTest.php`; `tests/Integration/MySql/Stage3ALiveSafetyMySqlTest.php` | `docs/03-DOMAIN_MODEL.md` → E9 / Stage 3-0 ExternalRecordLink | platform-reusable / family-hypothesis |
| Live execution | IMPLEMENTED (Stage 3A foundation shell only — no Adobe write, no merchant UI) | Connector | `app/Services/Sync/SyncLiveAdmissionService.php`; `app/Jobs/Connectors/SyncLiveRunJob.php` | `sync_runs` | `tests/Feature/Sync/Stage3ALiveSafetyFoundationTest.php` | `docs/03-DOMAIN_MODEL.md` → Stage 3-0 / Stage 3A–3E; Adobe Products/Export/Live support remains **false** until Stage 3E flip | connector-owned transport; platform-reusable outcomes |
...
| ExternalRecordLink (account-scoped) | IMPLEMENTED (Stage 3A foundation) | required | required (parent + children) | required (GraphQL GIDs) | channel offer ids | optional | platform-reusable / family-hypothesis |
