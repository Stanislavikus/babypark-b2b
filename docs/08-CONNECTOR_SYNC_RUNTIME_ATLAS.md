...
| Adobe Product desired-state compilation (Stage 3B-2) | IMPLEMENTED (unwired) | Connector | `app/Support/Connectors/AdobePaaS/Command/AdobeProductDesiredStateCompiler.php`; `AdobeProductDesiredState`; `AdobeProductCommandCompilationException` — compiles `AdobeProductExportSemanticResult` only; uses semantic-projected `external_field_key` provenance embedded in mapped values; blocks on semantic findings; uses `resolved_price.effective_net_price` + currency; protects connector-owned top-level fields | none | `tests/Unit/Connectors/AdobePaaS/Command/AdobeProductCommandFoundationUnitTest.php`; `tests/Feature/Sync/Stage3B2AdobeSimpleCommandSafetyTest.php` | `docs/03-DOMAIN_MODEL.md` → **3B — Adobe Simple Live** | connector-owned |
| Adobe Product remote state + GET classifier (Stage 3B-2) | IMPLEMENTED (unwired) | Connector | `app/Support/Connectors/AdobePaaS/Command/AdobeProductRemoteStateClient.php`; `AdobeProductCommandRequestFactory.php`; `AdobeProductRemoteGetClassifier.php`; `AdobeProductRemoteStateNormalizer.php`; `AdobeProductRemoteStateComparator.php` — store-scoped GET/POST/PUT via existing OAuth1 transport; `TRUSTED_KNOWN_MISSING` only when Magento JSON `parameters` contains exact SKU | none (read/write HTTP seam; not invoked by Live job) | `tests/Unit/Connectors/AdobePaaS/Command/AdobeProductCommandRequestFactoryTest.php`; `tests/Feature/Sync/Stage3B2AdobeSimpleCommandSafetyTest.php` | `docs/03-DOMAIN_MODEL.md` → Adobe external identity / reconciliation (Stage 3B-2) | connector-owned |
| Adobe Product simple command executor + ERL safety (Stage 3B-2) | IMPLEMENTED (unwired) | Connector | `app/Support/Connectors/AdobePaaS/Command/AdobeProductSimpleCommandExecutor.php`; `AdobeProductExternalRecordLinkGuard.php`; `AdobeProductExternalRecordLinkPersister.php`; `ConservativeAdobeProductOwnershipTrustPolicy.php` — no-link create (at most one POST), trusted-link update (at most one PUT), ambiguous write → one reconciliation GET, no second write; conservative ERL persistence (no fabricated ownership on POST) | `external_record_links` (existing Stage 3A schema) | `tests/Feature/Sync/Stage3B2AdobeSimpleCommandSafetyTest.php`; `tests/Feature/Sync/Stage3B2StructuralRegressionTest.php`; `tests/Feature/Sync/ExternalRecordLinkPersistenceTest.php` | `docs/03-DOMAIN_MODEL.md` → E9 ExternalRecordLink; Stage 3B-2 command safety | connector-owned |
...
| Live execution | IMPLEMENTED (Stage 3A safety shell only — no Adobe write, no merchant UI) | Connector | `SyncLiveAdmissionService`; `SyncLiveRunJob` fail-closed shell | `sync_runs` | `tests/Feature/Sync/Stage3ALiveSafetyFoundationTest.php` | `docs/03-DOMAIN_MODEL.md` → Stage 3-0 / Stage 3A–3E; merchant consequential exposure gated on `ConnectorSyncOperationSupport(Products, Export, Live) === true` until Stage 3E flip | connector-owned transport; platform-reusable outcomes |
