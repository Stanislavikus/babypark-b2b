...
The corrected Gate 1D normalization preserves the frozen applicability context (`product_type=LUGGAGE`, marketplace `ATVPDKIKX0DER`, requirements `LISTING`, parentage example and schema-version token) in `source_context_key` without inventing an applicability FK. PTD property presence is recorded as schema evidence, not as proof of live listing READ capability, and property writes remain PTD-conditioned rather than unconditional. Canonical mappings retain this exact evidence scope, while automatic suggestions now fail closed for applicability contexts more specific than `global`/`channel` until runtime has authoritative context to prove them; this is a precondition for future Amazon connector activation, not a current merchant-facing incident.

Identity, taxonomy, pricing, availability, compliance/media and variant-composition fates remain separate. `condition_type` is a reusable Variant/sellable-condition candidate owned by `ProductVariantData`, while `condition_note` remains Amazon listing context. `purchasable_offer` is explicitly a deferred structured multi-facet Pricing envelope; independently evidenced `list_price` remains a verified recommended-retail/reference-price representation. `bullet_point` remains a verified product-highlights representation while `special_feature` stays OPEN as a distinct sibling. `item_weight` and `item_package_weight` mappings are `partially_verified` because the frozen PTD evidence does not prove DEC-009's exact packaging boundaries. `number_of_items` is also deferred as a related Amazon branded-container/package-count representation rather than a verified generic `package_quantity` rename. The corrected pass contains 90 Amazon-local concepts and 14 disagreement families.
...
Two platform-ownership questions remain intentionally open in the Shopify shard: standard Product subtitle/short-title ownership and unit-pricing measure ownership. These rows are `DEFER_DECISION` / `DEFERRED_REVIEW`; the pass does not manufacture a Content, Pricing or Compliance contract merely to close the queue. Every structured unit-pricing member consistently uses `PricingOrCompliance` as its unresolved owner candidate without changing its structured fate or resolving that queue.
