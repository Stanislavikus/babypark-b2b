...
- Local physical/destructive MySQL tests on the shared project server must reuse the single designated disposable smoke database already configured for that environment. GitHub Actions uses its own ephemeral test database inside the workflow container. Do not create ad-hoc persistent test databases unless a concrete reproducer or certification case requires isolation; any such temporary database must be explicitly named for that case and removed after closure.
- Pull-request CI must use PR-scoped concurrency with `cancel-in-progress: true` so a newer `synchronize` run cancels the obsolete run for the previous PR HEAD instead of consuming a second runner for stale evidence.
