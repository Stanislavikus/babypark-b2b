...
- **Product Workbench / channel workspace (Resolved 2026-09-21; visual amendment 2026-09-22):** one Master Products catalogue remains editable truth, while a connected channel exposes distinct row universes. For Magento, `Огляд` opens on the current successful Remote Catalogue snapshot and `Публікація` shows local selected/linked Master Products. Correspondence stays visible through `Зв'язок`, filters/system views and governed row actions inside `Огляд`, not a permanent third top-level tab in first scope. Remote rows remain read-only provider observation and never become Master Products merely by appearing in `Огляд`. Publication rows keep correspondence, data state, readiness, problems, provider state and run result semantically separate. Projection V2 supplies the useful remote columns before implementation freezes their display. Normative detail: `PRODUCT_CHANNEL_SELECTION_REMOTE_CATALOGUE_CONTRACT.md` + `reviews/PRODUCT_WORKBENCH_STRUCTURAL_CONTRACT_2026_09_21.md` + `reviews/PRODUCT_WORKBENCH_VISUAL_UX_CONTRACT_2026_09_22.md`.
- **Mapping:** concept-first matrix with internal catalog/domain concepts as the primary rows; platform suggestions + discovery validation; not “map every external field.” Shipped on `ManageSyncFieldMappings` (Task 4C-1c-2b, PR #139). Select-field **Option Mapping** remediation shipped on nested `ManageSyncFieldOptionMappings` (Stage 2B) — uses existing `view_sync_mappings` / `manage_sync_mappings` permissions only; read from persisted connector snapshot metadata (zero HTTP on read); confirm/replace validate externally outside locked DB transaction; Preview findings remain historical after remediation; stale/orphan option-mapping cleanup is narrow and does **not** fix Product/Variant select value integrity.
