...
Normative sync domain shape and merchant journey are settled (Sync Domain Rebaseline in `03-DOMAIN_MODEL.md`). The connector UX contract still defines required behavior **when implemented** for: sync execution, Preview, enabled semantic operations, scheduling, ownership persistence (if bidirectional ships), issue aggregation, bulk resolution, sync-run history. Stage 1 Preview Engine is shipped (`run_sync_preview`, persisted zero-mutation Preview). Stage 2-0 freezes merchant Preview authorization/remediation (docs only). Stage 2A implements merchant Preview UI and `manage_sync_configurations`. Remaining work is implementation/UI migration, not another open architecture research pass, unless current repository truth contradicts an approved invariant.
