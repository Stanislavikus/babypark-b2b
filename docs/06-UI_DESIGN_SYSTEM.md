...
- **Product Workbench / channel workspace (Resolved 2026-09-21):** one Master Products catalogue remains editable truth, while a connected channel exposes distinct Views over different row universes. For Magento, `Огляд` opens on the current successful Remote Catalogue snapshot; `Публікація` shows local selected/linked Master Products; `Зв'язки` shows correspondence. Remote rows remain read-only provider observation and never become Master Products merely by appearing in `Огляд`. Publication rows keep correspondence, readiness, provider state and run result semantically separate. Current Remote Catalogue does not yet support default brand/category/thumbnail columns; Projection V2 must supply them before UI promises them. Normative detail: `PRODUCT_CHANNEL_SELECTION_REMOTE_CATALOGUE_CONTRACT.md` + `reviews/PRODUCT_WORKBENCH_STRUCTURAL_CONTRACT_2026_09_21.md`.
