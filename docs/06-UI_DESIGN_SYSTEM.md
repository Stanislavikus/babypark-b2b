...
- **Channel Product selection/workspace (Resolved 2026-09-15):** one Master Products catalogue; each destination is a selection/lens over it. Zero selection explains that Products must be chosen and offers one `Вибрати товари` action. Channel rows use compact Product identity (thumbnail/title/SKU/GTIN where available), keep remote correspondence separate from readiness, and collapse detailed findings behind blocker/recommendation counts. Provider remote-only records are a separate read-only projection, never a second editable Product catalogue. Normative detail: `PRODUCT_CHANNEL_SELECTION_REMOTE_CATALOGUE_CONTRACT.md`.
