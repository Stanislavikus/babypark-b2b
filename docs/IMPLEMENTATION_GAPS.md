...
| **Product→Channel Selection + Remote Catalogue Projection** | **Docs contract frozen 2026-09-15; runtime pending** — replace temporary `all_products` with SyncConfiguration-owned Product-level `explicit_products`; workspace-guarded normalized membership; every selection change advances configuration revision; Preview resolves one exact set; Live binds to one concrete current-revision Completed Preview set; channel workspace is a lens over Master Products; provider remote-only Products live in separate ConnectorAccount/target-owned immutable successful Remote Catalogue snapshots; failed/incomplete scans never publish; Magento >10k enumeration requires real-target proof; remote observations do not create Product/ExternalRecordLink trust; provider-neutral SEO/Search evidence and governed AI/media compatibility are frozen in `docs/PRODUCT_CHANNEL_SELECTION_REMOTE_CATALOGUE_CONTRACT.md` |
