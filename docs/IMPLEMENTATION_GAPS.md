...
| **Magento V1 Connection Truth & Monitoring** | **Frozen docs contract; runtime correction pending** — `docs/connectors/adobe-commerce/MAGENTO_V1_CONNECTION_UX_CONTRACT.md`: Product READ is the only healthy baseline; first-connect auto-check; structured conservative auth/authz remediation; `store_code=all` forbidden for V1; test-before-save OAuth1 quartet rotation; event-driven recovery + dated green; Product WRITE blocked is operation readiness, not connection failure. Current attribute-first/best-effort-catalog runtime, manual-only first check, direct credential overwrite, and missing merchant replacement/remediation flow are implementation gaps for the certification campaign. |
