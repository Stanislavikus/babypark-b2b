...
| **Stage 2A — Merchant Preview Core + Connector Setup** | **Stage 2A-1 Done** (sync setup authority + Adobe Products Export setup); **Stage 2A-2 pending** (merchant Preview result/worklist UI, completed-result summary, Needs-attention working set, contextual remediation, honest `NO_EDIT_SURFACE`, explicit rerun). Stage 2A remains pending/incomplete until Stage 2A-2. |
