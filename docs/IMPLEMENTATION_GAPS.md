...
| **Magento V1 Connection Truth & Monitoring** | **Frozen docs contract; implementation in progress** — `docs/connectors/adobe-commerce/MAGENTO_V1_CONNECTION_UX_CONTRACT.md`: Product READ is the only healthy baseline; first-connect auto-check; structured conservative auth/authz remediation; `store_code=all` forbidden for V1; test-before-save OAuth1 quartet rotation; event-driven recovery + dated green; Product WRITE blocked is operation readiness, not connection failure. **Product READ baseline correction, first-connect auto-check, and conservative access classification are implemented on the certification branch**: connection check uses one bounded `/V1/products?pageSize=1` baseline; new connections enqueue an auditable `first_connect` check; validated Magento `parameters.resources` evidence classifies 401/403 as authorization while ambiguous 401/403 remains an explicit user-action-required unknown instead of inventing bad credentials or missing permissions. Remaining gaps: V1 `all` guard, safe credential replacement flow, recovery wiring, and merchant operation-specific WRITE-paused/remediation presentation. |
