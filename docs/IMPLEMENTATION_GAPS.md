...
- `SyncRun` / `SyncRunItem` persistence and revision v3 rebaseline are
  implemented (Task 4C-2b-1, Done). `FieldMapping` persistence/manual
  confirmation and authoritative-discovery validation are implemented (Task
  4C-1b, Done). Canonical suggestion/read-model and UI-prefill work is Task
  4C-1c (4C-1c-0 docs contract frozen; 4C-1c-1 provider/read-model Done;
  4C-1c-2a authorization contract Done; 4C-1c-2b Layer B mapping UI Done, PR
  #139). Preview-first execution foundation contract is frozen (Task 4C-2a,
  docs only). Magento Product Export V1 execution contract is frozen (Platform
  Product Scope Rebaseline, docs only): `run_sync_preview` eighth permission
  (runtime catalogue eight permissions — Stage 1 Done), Product execution
  aggregate, mode-aware support, FieldOptionMapping, Preview
  admission/execution, connector execution configuration + revision v4 (Stage
  1 Done).
- **Historical (pre-Stage 3A):** `ExternalRecordLink`, Live permission
  (`run_sync_live`), Live admission/execution mechanics, stale active-run
  recovery, scheduling, sync history/issues, and merchant Live UX beyond
  Preview remained absent; Stage 3-0 docs contract was **Done**.
- **Current (post-Stage 3A):** Stage 3A Live Safety foundation is **Done** —
  ...
  `ExternalRecordLink` persistence foundation, and `SyncLiveAdmissionService`.
  **Stage 3B-1** shared Adobe semantic boundary is **Done**. **Stage 3B-2**
  Adobe Product simple command safety foundation is **Done** and wired through
  Stage 3B integration. **Stage 3B — Adobe Simple Live** is **Done and public for the certified Magento V1 scope** — generic `SyncLiveRunJob` orchestration with `SyncLiveConnectorCapability`, Adobe `live_capability`, store-config `base_currency_code` proof, DB-fresh writer-lease gate, and fresh runtime-readiness at admission + worker. Adobe Products/Export/Live advertised support is **true** after real-target E2E certification on 2026-09-19; **Stage 3C** (Configurable Live), **Stage 3D-1** (E14 media Live runtime), and normative **Stage 3D** (merchant first-Live UX) are **Done** for their bounded certified semantics;
  **Stage 3E** (entity-bound Safe Sync runtime contract **Done (docs contract)**;
  Safe Sync read + isolated simple WRITE foundation retained internally as optional Enhanced Safety;
  standard trusted Product UPDATE now uses the moduleless stock REST path with MerchantConfirmed identity, bounded verified writes, reconciliation, and fresh runtime readiness; the full bounded Export Live campaign is real-target E2E certified as of 2026-09-19;
  bounded Safe Sync remains optional Enhanced Safety rather than a standard-path prerequisite;
  Products/Import/Live remains **false**, Product CREATE remains unsupported, and connector-specific deferred certification items are explicitly non-blocking for the certified Export Live scope).
  Scheduling,
...
| **Stage 3B-2 — Adobe Product simple command safety foundation** | **Done + consumed by public bounded Export Live** — `AdobeProductDesiredStateCompiler`, remote state client/classifier, ERL guard/persister, `AdobeProductSimpleCommandExecutor`; fake-HTTP tests; wired through Adobe `live_capability` into generic `SyncLiveRunJob`; **historical no-link create path superseded by Stage 3E Stop-and-Amend** (stock Magento POST is upsert-like; link-first required); Product CREATE remains unsupported |
| **Stage 3B — Adobe Simple Live integration** | **Done + public for the certified Magento V1 scope** — generic Live capability seam (`SyncLiveConnectorCapability`, resolver, `live_capability` profile binding); `SyncLiveRunJob` orchestration; `AdobeProductExportLiveCapability`; shared run metadata preparer; `AdobeStoreConfigReader`; fresh `ConnectorLiveRuntimeReadiness`; writer-lease consequential-write gate; fake-HTTP regression matrix; real-target E2E verified 2026-09-19 |
| **Stage 3C — Adobe Configurable Live** | **Done + public for certified bounded Configurable semantics** — configurable command/runtime foundation; historical deterministic-parent generator remains only for a future out-of-V1 CREATE capability; Magento V1 itself is existing-family link/update-only and must fail closed for missing parent/child identity |
| **Stage 3D-1 — Adobe E14 media Live runtime** | **Done + public for certified media semantics** — `ProductExecutionImageInput` from `products.images`; Adobe primary/gallery media Live executor wired through `AdobeProductExportLiveCapability`; verified source fetch + remote metadata/content comparison; E14 runtime consumer exists; unsupported media-clear edge cases remain fail-closed/deferred |
| **Stage 3D-2 — Merchant first-Live UI/read model** | **Done + public/actionable for Adobe Products Export Live** — dual-authority `ManageAdobeProductsExportPreview`; `SyncLiveMerchantReadService`; Live worklist/result presentation; `startLive()` delegates to `SyncLiveAdmissionService`; transient runtime-readiness failure is surfaced to the merchant without creating a run |
| **Stage 3D — Adobe Media + Merchant First Live** | **Done + public for the certified Magento V1 Export Live scope** — 3D-1 E14 media runtime + 3D-2 merchant Live admission UI/read model |
| **Stage 3E — Real Adobe Validation + Truth Flip** | **Done + public truth flip completed 2026-09-19** — historical entity-bound Safe Sync and Post-#168 amendments remain recorded in `docs/03-DOMAIN_MODEL.md`; standard Magento V1 is moduleless-by-default, MerchantConfirmed link/update-only, no Product CREATE, with optional Safe Sync Enhanced Safety. P-01/P-02/P-03/P-10/P-11 and Configurable/media/lifecycle paths have real-target evidence; fresh `ConnectorLiveRuntimeReadiness` runs at admission and worker, followed by writer-gate recheck. Bounded real-target E2E no-op, one-PUT change, restore, and zero-write currency mismatch are certified in `magento_v1_products_export_live_certification_2026_09_19.json`. Products/Export/Live = **true**; Products/Import/Live = **false**. |
| **Stage 3E-R2b-1 — Merchant ENTITY TRUST backend** | **Done (internal)** — review/confirm services, link readiness projector, candidate discovery + Safe Sync verification, encrypted review envelope (v1: logical entity + fingerprint + explicit relink + target snapshot), configurable-family atomic persistence, ConnectorAccount target-freeze guard; explicit relink backend semantics; zero Magento consequential writes |
| **Stage 3E-R2b-2 — Merchant ENTITY TRUST UI** | **Done + public in certified Export Live** — `ManageAdobeProductsExportPreview` per-item review/relink/confirm surface; server-side working-set re-projection for product/readiness/family authority; opaque `EntityTrustReviewFlowStore`; per-product parent-SKU merchant input; truthful media/success copy; zero Magento consequential writes during trust confirmation; actionable identity exceptions surface under public Export Live eligibility |
| **Stage 3E-R2b-3 — Magento V1 merchant journey rebaseline** | **Docs contract frozen 2026-09-14; runtime pending** — LINK/UPDATE-ONLY capability truth; aggregate identical configuration-level Preview blockers in presentation while preserving per-item evidence; causal remediation routing by owning domain; one ordered merchant attention journey instead of two equal Preview/Entity-Trust task tables; hide/collapse Entity Trust preparation from the default merchant journey while Live support is false; after truth flip show actionable identity exceptions only; explicit Magento account/store target context; no-create merchant copy; distinct configurable missing-parent-SKU/relink reason; no change to FieldMapping/Entity Trust domain separation or fail-closed identity runtime |
| **Product→Channel Selection + Remote Catalogue Projection** | **Runtime implemented through merchant linking candidate integration; production-readiness evidence still pending** — PR #220 replaced temporary `all_products` with SyncConfiguration-owned Product-level `explicit_products`, workspace-guarded normalized membership, revision-aware exact Preview membership, concrete Preview→Live source binding, Master Products channel assignment/workspace, immutable successful Remote Catalogue snapshots, and >10k-safe keyset enumeration implementation. The follow-up linking campaign connects current remote-only Magento rows to the existing merchant-confirmed Entity Trust review/confirm flow without creating Products or issuing Magento writes; selected Remote Catalogue `entity_id` is checked against the fresh verified Magento subject before review evidence is issued, and only existing `ExternalRecordLink` persistence remains trust authority. The visible catalogue journey now exposes the Master Catalogue entry, Magento channel workspace, and the full current Magento catalogue with linked/unlinked status and filtering while preserving Entity Trust as the only link authority. Magento >10k enumeration still requires representative real-target proof before production-readiness claims, and deeper per-Product readiness/remediation polish remains separate pending work. Adobe Products/Export/Live is now **true** for the certified bounded scope; >10k catalogue enumeration evidence and deeper catalogue polish remain deferred/non-blocking for Export Live. Normative contract remains `docs/PRODUCT_CHANNEL_SELECTION_REMOTE_CATALOGUE_CONTRACT.md`. |
| **Magento V1 Connection Truth & Monitoring** | **Frozen docs contract; implementation in progress** — `docs/connectors/adobe-commerce/MAGENTO_V1_CONNECTION_UX_CONTRACT.md`: Product READ is the only healthy baseline; first-connect auto-check; structured conservative auth/authz remediation; `store_code=all` forbidden for V1; test-before-save OAuth1 quartet rotation; event-driven recovery + dated green; Product WRITE blocked is operation readiness, not connection failure. **Product READ baseline correction, first-connect auto-check, conservative access classification, reserved-store guard, and the safe credential-rotation backend are implemented on the certification branch**: connection check uses one bounded `/V1/products?pageSize=1` baseline; new connections enqueue an auditable `first_connect` check; validated Magento `parameters.resources` evidence classifies 401/403 as authorization while ambiguous 401/403 remains an explicit user-action-required unknown instead of inventing bad credentials or missing permissions; reserved `all` store scope is rejected case-insensitively. Credential replacement now tests quartet B transiently before persistence, preserves A on validation failure, serializes the commit with the shared connector-account job lock, fresh-checks target/settings/credential generation, preserves Entity Trust, and records successful B evidence as `credentials_replacement`; direct generic update replacement is blocked. The Magento Overview now exposes manager-only full-quartet credential replacement without rendering stored secrets; failed B preserves A and shows only safe remediation, while successful replacement records auditable verification evidence. Transient connection recovery is now event-driven: a terminal automatic-retry failure schedules evidence-bound system rechecks with finite 1h/2h/4h/8h/16h/32h backoff, never polls healthy accounts, aborts when newer connection evidence supersedes the source failure, and stops after six recovery cycles. Remaining gap in this slice: merchant operation-specific WRITE-paused/remediation presentation. |
...
Schedule and sync-run history remain unimplemented. `run_sync_preview` runtime
permission is implemented (Stage 1; eighth catalogue permission). Adobe
`(products, export)` Preview and certified bounded Live support are declared; public Export Live truth is **true** while Import Live remains **false**. Stage 2A merchant Preview UI and Stage 2A-1 `manage_sync_configurations` runtime permission are implemented. Stage 2B Option Mapping remediation UI is implemented (`ManageSyncFieldOptionMappings`). Stage 2-0 merchant Preview authorization/remediation contract is **Done (docs contract)**. Stage 3A Live Safety foundation is **Done** — `run_sync_live` (tenth permission), stale active-run recovery, `ExternalRecordLink` persistence foundation, and `SyncLiveAdmissionService`. Stage 3B Adobe Simple Live integration is **Done and public for the certified scope** — generic `SyncLiveRunJob` orchestration, Adobe `live_capability`, fresh runtime readiness, and real-target E2E verification.
Runtime catalogue is **ten** permissions (`run_sync_live` landed Stage 3A;
`manage_sync_configurations` landed Stage 2A-1). Connector-account **creation UI is implemented**
...
internal; 3D-2 merchant first-Live **Done internal**) → Stage 3E
...
schema fields / change inspection), retention/pruning (4B-2d), Stage 3E support-truth
...
| **B. Implementation gaps** | Magento Products/Export/Live truth flip is **Done and real-target E2E certified 2026-09-19**. Stage 3D merchant first-Live UX is public/actionable for the certified scope; Stage 3C Configurable Live and Stage 3D-1 media runtime are public only for their certified bounded semantics; Stage 3B shared semantics/simple Live and Stage 3A Live Safety are implemented. Products/Import/Live remains false, Product CREATE remains unsupported, and connector-specific deferred follow-ups remain explicit rather than blocking Export Live. Stage 1 Preview Engine, Stage 2A merchant Preview, Stage 2B Option Mapping, and `SyncRun`/`SyncRunItem` persistence are implemented. Connector-account **create** UI is implemented. | No for certified Magento Export Live; yes only for deferred capabilities such as public Import Live |
...
- **Shipped (Stage 2B):** Option Mapping remediation UI on
  ...
  persisted connector snapshot metadata on read (zero HTTP); verified Canonical
  Registry option evidence can produce transient fail-closed 1:1 suggestions and
  action prefill only, with existing mappings reserving external values;
  `confirm`/`replace` retain connector external validation outside locked DB
  transaction and merchant confirmation remains authoritative; Preview findings
  remain historical after remediation; narrow stale/orphan option-mapping cleanup
  (does **not** fix Product/Variant select value integrity).
...
- Stage 3A Live Safety foundation: `run_sync_live`, stale active-run recovery,
  `ExternalRecordLink` persistence, `SyncLiveAdmissionService`.
- Stage 3B-1 shared Adobe semantic boundary (Done).
- Stage 3B-2 Adobe Product simple command safety foundation (Done, wired through
  Stage 3B): `AdobeProductDesiredStateCompiler`, remote state client/classifier,
  `AdobeProductSimpleCommandExecutor`, ERL guard/persister with conservative
  ownership policy; comprehensive fake-HTTP tests; wired through generic
  `SyncLiveRunJob` via Adobe `live_capability`.
- Stage 3B Adobe Simple Live integration is **Done and public for the certified Magento V1 scope**: generic Live capability seam, `SyncLiveRunJob` orchestration, `AdobeProductExportLiveCapability`, shared run metadata preparer, `AdobeStoreConfigReader`, DB-fresh writer-lease consequential-write gate, and fresh `ConnectorLiveRuntimeReadiness` at admission + worker. Real-target end-to-end no-op, verified one-PUT change, verified restore, and fail-closed currency mismatch were certified on 2026-09-19. Adobe Products/Export/Live advertised support is **true**; Products/Import/Live remains **false** and Product CREATE remains unsupported in V1.
...
- mode-aware execution support (Adobe Products/Export: Preview and certified bounded Live supported; Import Live not supported);
...
**Approved docs:**
- `03-DOMAIN_MODEL.md`, Receive / Import Foundation Contract (Resolved): A generic governed Product/Variant field-value writer handles ordinary dynamic `FieldBinding` values whose invariants belong entirely to Field Dictionary during Receive operations. Current generic scope: `Text`, `LongText`, `Number`, `Decimal`, `Boolean`, `Date`, single-value `Select`, `MultiSelect`, and `Url`.

**Current code:**
- GAP-028 is implemented as the platform-core governed writer for ordinary Product/Variant dynamic `Text`, `LongText`, `Number`, `Decimal`, `Boolean`, `Date`, single-value `Select`, `MultiSelect`, and `Url` fields. See `app/Services/Fields/GovernedDynamicFieldValueWriter.php` and the bounded typed exception set under `app/Services/Fields/Exceptions/`. Coverage includes explicit Set/Clear commands, workspace + object-type eligibility, fail-closed metadata/rule validation, internal-option validation, localized text merge semantics, exact typed canonical storage, and narrow same-slot concurrency via `DB::transaction` + `lockForUpdate` with bounded retry on duplicate-slot races.
...
GAP-028 provides the governed Product/Variant dynamic writer for Text,
LongText, Number, Decimal, Boolean, Date, single-value Select,
MultiSelect, and Url, including workspace/object-type validation,
explicit clear, localized text merge semantics for text types, typed
canonical storage, and safe same-slot concurrency.
