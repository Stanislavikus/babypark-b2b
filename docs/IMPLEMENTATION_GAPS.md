...
| **Magento V1 Connection Truth & Monitoring** | **Frozen docs contract; implementation in progress** — `docs/connectors/adobe-commerce/MAGENTO_V1_CONNECTION_UX_CONTRACT.md`: Product READ is the only healthy baseline; first-connect auto-check; structured conservative auth/authz remediation; `store_code=all` forbidden for V1; test-before-save OAuth1 quartet rotation; event-driven recovery + dated green; Product WRITE blocked is operation readiness, not connection failure. **Product READ baseline correction and first-connect auto-check are implemented on the certification branch**: connection check now uses one bounded `/V1/products?pageSize=1` baseline, derives catalogue evidence from that same response, keeps media optional/non-authoritative, and new connections immediately enqueue an auditable `first_connect` check through the existing retry/persistence pipeline. Remaining gaps: structured auth/authz remediation, V1 `all` guard, safe credential replacement flow, recovery wiring, and merchant WRITE-paused/remediation presentation. |
