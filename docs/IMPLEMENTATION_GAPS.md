...
- **Current (post-Stage 3A):** Stage 3A Live Safety foundation is **Done** —
  ...
  **Stage 3B-1** shared Adobe semantic boundary is **Done**. **Stage 3B-2**
  ...
  Stage 3B integration. **Stage 3B — Adobe Simple Live** integration is **Done
  (internal)** — generic `SyncLiveRunJob` orchestration with
  `SyncLiveConnectorCapability` seam and Adobe simple `live_capability`
  ...
  **Stage 3E** (entity-bound Safe Sync runtime contract **Done (docs contract)**;
  ...
  standard trusted simple Product WRITE now uses the moduleless stock REST GET/PUT/GET path internally; the core path and base-price restore cycle were real-target verified on 2026-09-11, while field-by-field certification remains pending;
  bounded Safe Sync module correction remains implemented internally as optional Enhanced Safety;
  moduleless Simple core is real-target certified for trusted base-price WRITE/verify/restore, while field-by-field Adobe WRITE validation, configurable/media completion, and truth flip remain **pending**; stock no-link auto-create remains invalidated and merchant-confirmed Entity Trust stays mandatory before standard-path mutation).
  Scheduling,
...
| **Stage 3B-2 — Adobe Product simple command safety foundation** | **Done** — `AdobeProductDesiredStateCompiler`, remote state client/classifier, ERL guard/persister, `AdobeProductSimpleCommandExecutor`; fake-HTTP tests; wired through Adobe `live_capability` into generic `SyncLiveRunJob`; **historical no-link create path superseded by Stage 3E Stop-and-Amend** (stock Magento POST is upsert-like; link-first required); Adobe Products/Export/Live support remains **false** |
...
| **Stage 3C — Adobe Configurable Live** | **Done (internal)** — configurable command/runtime foundation; historical deterministic-parent generator remains only for a future out-of-V1 CREATE capability; Magento V1 itself is existing-family link/update-only and must fail closed for missing parent/child identity |
...
| **Stage 3E — Real Adobe Validation + Truth Flip** | **Done (docs contract)** — entity-bound Safe Sync runtime contract frozen after Security/Concurrency arbitration; first-party Magento component + ENTITY TRUST + account readiness freeze documented; PR #160 runtime reverted; **Stage 3E-R1 internal read foundation is implemented** (standalone Magento Safe Sync module, authenticated handshake, entity-bound Product verification/read, bounded Laravel client, Galera causal-read seam); **standard moduleless trusted simple Product WRITE consumption is implemented internally** (`AdobeProductSimpleCommandExecutor` now delegates to the stock REST writer after trusted-link, discriminator, SKU, and consequential-write gate checks; the stock path proves remote Magento `id` against the merchant-confirmed discriminator, performs at most one PUT, never POST/create, and verifies/reconciles by GET; controlled custom scalar comparison tolerates only bounded canonical Magento round-trips, while mapped custom-attribute clear intent fails closed before PUT when a stale remote value remains because stock clear semantics are still pending real-target certification); **Safe Sync simple WRITE remains implemented as an optional Enhanced Safety primitive and is no longer the standard-path dependency**; **Stage 3E-R2a ERL provenance foundation is implemented**; **Stage 3E-R2b-1 merchant-confirmed ENTITY TRUST review/confirm backend is implemented**; **Stage 3E-R2b-2 merchant per-item informed review/confirm UI is implemented** with an opaque server-side review-flow store that keeps `reviewToken` out of browser state; browser state is presentation/input only and not identity authority; **disposable validation harness is implemented internally as a validation-only Laravel control plane**; real-target core Simple price WRITE/restore step is verified; remaining field-by-field validation, decisions 5–9 as applicable, configurable/media/live-support completion, and support flip remain **pending**; support remains **false**; **Stage 3E Post-#168 Real-Target Certification Amendment (docs only) is recorded** in `docs/03-DOMAIN_MODEL.md` → 9 decisions (current state, media-neutral Product save, connection quarantine, price scale 6 not a defect, store view context amending E12, PHP / Adobe certification matrix, callback failure semantics, content staging, order of work) plus a `Code-vs-docs dormant discrepancies` table that records the still-present stock SKU-addressed media / configurable / lifecycle code paths as production-unreachable and to be replaced before their respective Live path becomes reachable; no change to the first-party `integrations/magento-safe-sync` module in this campaign; Laravel trusted simple execution now consumes the moduleless stock REST writer; Safe Sync remains optional Enhanced Safety; no `composer.json` change; no real-target validation harness execution/certification in this PR; no Live support enablement; no deployment |
...
| **Stage 3E-R2b-2 — Merchant ENTITY TRUST UI** | **Done (internal)** — `ManageAdobeProductsExportPreview` per-item review/relink/confirm surface; server-side working-set re-projection for product/readiness/family authority; opaque `EntityTrustReviewFlowStore`; per-product parent-SKU merchant input; truthful media/success copy; zero Magento consequential writes; Adobe Products/Export/Live support remains **false** |
...
`(products, export)` Preview support is declared; Live remains unsupported
(public support truth **false**). Stage 2A merchant Preview UI and Stage 2A-1
`manage_sync_configurations` runtime permission are implemented. Stage 2B Option
Mapping remediation UI is implemented (`ManageSyncFieldOptionMappings`). Stage
2-0 merchant Preview authorization/remediation contract is **Done (docs
contract)**. Stage 3A Live Safety foundation is **Done** — `run_sync_live`
(tenth permission), stale active-run recovery, `ExternalRecordLink` persistence
foundation, and `SyncLiveAdmissionService`. Stage 3B Adobe Simple Live integration
is **Done (internal)** — generic `SyncLiveRunJob` orchestration and Adobe simple
`live_capability` binding; merchant Live unreachable until Stage 3E.
...
| **B. Implementation gaps** | Stage 3E Adobe Live support flip. Stage 3D merchant first-Live UX **Done (internal)** — 3D-1 E14 media runtime + 3D-2 merchant Live UI/read model (non-actionable while Live support is **false**). Stage 3C Adobe Configurable Live integration **Done (internal)**. Stage 3B-1 shared Adobe semantics **Done**. Stage 3B-2 Adobe Product simple command safety foundation **Done** (wired through Stage 3B). Stage 3B Adobe Simple Live integration **Done (internal)** — generic `SyncLiveRunJob` orchestration and Adobe simple `live_capability` binding; merchant consequential Live unreachable until Stage 3E support flip. Stage 3A Live Safety foundation **Done** (`run_sync_live`, stale active-run recovery, `ExternalRecordLink` persistence, Live admission). Stage 3-0 docs contract **Done**. Stage 1 Preview Engine, Stage 2A merchant Preview, and Stage 2B Option Mapping remediation are implemented. `SyncRun`/`SyncRunItem` persistence is implemented (4C-2b-1). Connector-account **create** UI is implemented. | Yes for shipping sync; docs are settled |
...
- Stage 3B Adobe Simple Live integration (Done, internal): generic Live
  capability seam, `SyncLiveRunJob` orchestration, `AdobeProductExportLiveCapability`,
  shared run metadata preparer, `AdobeStoreConfigReader`, DB-fresh writer-lease
  consequential-write gate; Adobe Products/Export/Live advertised support remains
  **false**; merchant Live unreachable until Stage 3E.
